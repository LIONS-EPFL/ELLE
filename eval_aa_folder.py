...
def parser_eval():
    ...
    parser.add_argument('--folder', type=str, default='FAST_at_cosinew_scheduler_comparison')
...
for experiment in tqdm.tqdm(os.listdir(args.folder)):
    ...
    if 'pert' in experiment:
        ...
        eps=8
    if os.path.exists(os.path.join(args.folder,experiment,f'log-aa_{args.early_stopping}.log')):
        print('Already exists')
    elif os.path.exists(os.path.join(args.folder,experiment,'val_best.pt')):

        bash_cmd = f'CUDA_VISIBLE_DEVICES={args.gpu} python3 eval_aa.py --eps_eval {eps} --fname_input ' + os.path.join(args.folder,experiment) + f' --early_stopping {args.early_stopping}'
        os.system(bash_cmd)
